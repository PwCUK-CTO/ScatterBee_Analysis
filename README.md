...
insert image of red warning window...

These are caused by IDA failing to recognise how the stack is handled during the calls to the API lookup functions, you can see how IDA changes the value of the stack incorrectly at one of these calls in this screenshot:
...
insert IDA image with click here annotation...

Copy the value of this memory location and open the IDA segments window. `Shift+F7`  
...
insert images with examples...x2

Make sure to follow the code xrefs in IDA back to the entry point of the function before running the rebuild script so that the final rebuilt payload's first function is accurate.  
...
To decode these strings and add a comment with the decoded string at the location of the decode navigate into the decoding subroutine and work out if the algorithm uses a subtraction constant, like below, or an AES algorithm.  
